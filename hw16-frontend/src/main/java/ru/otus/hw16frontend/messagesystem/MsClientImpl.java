...
import ru.otus.hw16frontend.msocket.Client;
...
public class MsClientImpl implements MsClient {
    ...
    public MsClientImpl(@Value("${service.frontend.name}") String name, Client clientSocket) {
        this.name = name;
    ...
    public boolean sendMessage(Message msg) {
        boolean result = clientSocket.newMessage(msg);
        if (!result) {
